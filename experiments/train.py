...
def parse_args():
    ...
    parser.add_argument(
        "--max-episode-len",
        type=int,
        # probably a bit too large; original value was 25, which seems more
        # sane
        default=200,
        help="maximum episode length")
    parser.add_argument(
        # 30000 wasn't quite enough to catch all the variation on adv_gravity;
        # going a bit higher instead
        "--num-episodes", type=int, default=50000, help="number of episodes")
    parser.add_argument(
    ...
    parser.add_argument(
        "--save-root",
        default="./data/",
        help="root directory for saving files")
    parser.add_argument(
        "--benchmark-dir",
        type=str,
    ...
    parser.add_argument(
        "--save-dir",
        type=str,
        default="maddpg_policy/",
        help="subdirectory in which training state and model should be saved")
    parser.add_argument(
        "--save-rate",
        type=int,
        default=1024,
...
def get_trainers(env, num_adversaries, obs_shape_n, arglist):
    ...
    assert arglist.good_policy in {'maddpg', 'ddpg'}
    assert arglist.adv_policy in {'maddpg', 'ddpg'}
    num_good = env.n - num_adversaries
    for i in range(num_good):
        # first few agents are protagonists
        trainers.append(
            trainer(
                "agent_%d" % i,
                model,
    ...
    for i in range(num_good, env.n):
        # last few agents are antagonists
        trainers.append(
            trainer(
                "agent_%d" % i,
                model,
...
def train(arglist):
    with U.single_threaded_session():
        ...
        if not arglist.load and arglist.restore:
            # get latest snapshot
        ...
        final_ep_n_obs = []  # num of observations corresponding to above
        final_ep_n_epis = []  # num of episodes corresponding to above
        ...
        episode_step = 0
        train_step = 0
        # number of episodes seen at time of last save
        last_save = 0
        t_start = time.time()
        ...
        while True:
            ...
            if (done or terminal) \
               and len(episode_rewards) - last_save > arglist.save_rate:
                U.save_state(
                    os.path.join(arglist.save_dir, 'snapshot'),
                    saver=saver,
                    global_step=len(episode_rewards))
                # print statement depends on whether or not there are
                # adversaries
                if num_adversaries == 0:
                    # cooperative scenario, so all agents have same reward
                    print(
                    ...
                    # competitive scenario (agents might have different reward
                    # functions), so print all agent returns
                    print(
                ...
                final_ep_rewards.append(
                    np.mean(episode_rewards[-arglist.save_rate:]))
                final_ep_n_obs.append(train_step)
                final_ep_n_epis.append(episodes_seen)
                for rew in agent_rewards:
                    final_ep_ag_rewards.append(
                        np.mean(rew[-arglist.save_rate:]))
                last_save = len(episode_rewards)

            # saves final episode reward for plotting training curve later
            if len(episode_rewards) > arglist.num_episodes:
                rew_file_name = os.path.join(
                    arglist.plots_dir, arglist.exp_name + '_rewards.pkl')
                os.makedirs(os.path.dirname(rew_file_name), exist_ok=True)
                with open(rew_file_name, 'wb') as fp:
                    pickle.dump({
                        'final_per_agent': final_ep_rewards,
                        'final_means': final_ep_ag_rewards,
                        'final_eps_seen': final_ep_n_epis,
                        'final_obs_seen': final_ep_n_obs,
                    }, fp)
